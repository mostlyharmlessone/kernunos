...
!  each matching name has the value at origin, min value and max value
   ...
   INTEGER, ALLOCATABLE :: MV(:)
   REAL (wp) :: R0,THT0,Z0(3),SAGC0(3),INSTC0(3),INSTC20(3),MEANC0(3),MONGEA0(3),Warp0(3)
...
subroutine init_mat_JMatrix(MM,N,b) ! allocate common storage arrays
  ...
  allocate (b%MV(MM),b%RC(3,MM),b%Warp(N+1,MM))
...
subroutine Skyline_Test(Skyline, Penta)      ! tests splining on Penta with Skyline
  TYPE(wpSkyline), INTENT(INOUT) :: Skyline
  TYPE(wpPentaMatrix), INTENT(IN) :: Penta
  integer :: i,j,k,kk,L2,offset,NP,ITH
  real(wp) :: DAT,u,v,xx,yy,f,fTmp(Skyline%rows),f2Tmp(Skyline%rows)
  real(wp) :: z(Skyline%cols),z2(Skyline%cols)
  real(wp) :: x(Skyline%cols)              ! maximum size needed, don't need NP
  real(wp) :: y(Skyline%rows)
  real(wp) :: check,firstcheck,secondcheck
  NP=size(Skyline%DAT,1)
! Spline in x                               (this is the equivalent of DiaSpline)
  do i=1,Skyline%rows
   L2=Skyline%L2x(i)
...
! make a new grid
   do i=1,NP
    do j=1,NP
    u=-7.00+((i-1)*14.00)/(NP-1.0)
    v=-7.00+((j-1)*14.00)/(NP-1.0)
!    Populate JMatrix with Splined PentaCam
!    Spline in y      (this is the equivalent of Spline1Dx1D)
     firstcheck=0
     do k=1,Skyline%rows
...
end subroutine Skyline_Test

subroutine Spline_Test(Penta)      ! tests splining on Penta without borders
  TYPE(wpPentaMatrix), INTENT(IN) :: Penta
  integer :: i,j,k,kk,NP,ITH
  real(wp) :: DAT,u,v,xx,yy,f,fTmp(size(Penta%DAT,1)),f2Tmp(size(Penta%DAT,1))
  real(wp) :: z(size(Penta%DAT,1)),z2(size(Penta%DAT,1))
  real(wp) :: x(size(Penta%DAT,1)),allz2(size(Penta%DAT,1),size(Penta%DAT,1))
  real(wp) :: y(size(Penta%DAT,1))
  real(wp) :: check,firstcheck,secondcheck
  NP=size(Penta%DAT,1)
! setup x,y
  do i=1,NP
   x(i)=-7.00+((i-1)*14.00)/(NP-1.0)
   y(i)=7.00-((i-1)*14.00)/(NP-1.0)
  end do
! Spline in x
  do i=1,NP
   z(:)=Penta%DAT(i,:)
   call nspline(x(:),z(:),NP,z2(:))
   allz2(i,:)=z2(:)
  end do
  do i=1,NP
    do j=1,NP
     u=-7.00+((i-1)*14.00)/(NP-1.0) !compute on knots on a grid, not radial
     v=-7.00+((j-1)*14.00)/(NP-1.0)
!    Spline in y      (this is the equivalent of Spline1Dx1D)
     firstcheck=0
     do k=1,NP
      !x(:) is always the same here, no skyline
      z(:)=Penta%DAT(k,:)
      z2(:)=allz2(k,:)
!    f is value at u, fTmp is a new column of values at u (which since I'm on knots should be identical)
      call SplineEval(0,x(:),z(:),z2(:),NP,u,f)   ! first parameter = 0 nonperiodic
      call SplineEval(2,x(:),z(:),z2(:),NP,u,check)   ! first parameter = 2 extrapolation check
      if (check == 0) firstcheck=firstcheck+1 ! how much extrapolation
      fTmp(k)=f
     end do
     ...
     call SplineEval(0,y(:),fTmp(:),f2Tmp(:),NP,v,DAT)  ! first parameter = 0 nonperiodic
     call SplineEval(2,y(:),fTmp(:),f2Tmp(:),NP,v,secondcheck)  ! first parameter = 2 extrapolation check
!   boundary check here
     xx=u*(NP-1)/14.0 ; yy=v*(NP-1)/14.0
     if (Penta%DAT(1+(NP-1)/2+sign(floor(ABS(xx)),floor(xx)),&
                &1+(NP-1)/2+sign(floor(ABS(yy)),floor(yy))) > 0) then  ! test for >0 is why Penta needed here
