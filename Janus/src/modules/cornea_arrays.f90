...
!  Computed results: R is from make rings or Penta version; need to decalre one of each of these for each data set for comparison
   REAL (wp), ALLOCATABLE :: R(:,:),Z(:,:),DEG(:),SAGC(:,:),INSTC(:,:),MEANC(:,:),MONGEA(:,:)
   REAL (wp), ALLOCATABLE :: MV(:),RC(:),LIOC(:,:) !last one is MM*N,4  RC is RadSplineCenter, compare to R0
   REAL (wp) :: R0, Z0(3), DEG0, SAGC0(3), INSTC0(3), MEANC0(3), MONGEA0(3)  ! central values and powmin & powmax
 END TYPE wpJMatrix
  ...
  do i=1,MM
    Atlas%DEG(i)=RadSlope%thta(i)
      ...
      if (ABS(YP) > 0._wp) then
       ...
       Atlas%AY(i,imv(i))=RadSlope%Z(i,j)
      endif
      end do
  end do
