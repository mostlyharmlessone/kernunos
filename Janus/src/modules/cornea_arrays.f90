...
!   boundary check here 
    ...
    if (Penta%CUR(1+(NP-1)/2+sign(floor(ABS(xx)),floor(xx)),1+(NP-1)/2+sign(floor(ABS(yy)),floor(yy))) > 0) then  ! test for -1 is why Penta needed here
...
subroutine RadSlope_eq_Atlas(JMatrix,RadSlope,Atlas) ! initially populates r, thta, Zp, MV
  ...
  INTEGER :: i,j,MM,N,imv(size(RadSlope%r,2))
    ...
    do i=1,MM
     ...
     ! RadSlope%thta(i)=PI*2*(i-1)/180_wp should always be true for MM=180
     ! RadSlope%thta(i)=PI*(i-1)/180_wp should always be true for MM=360
     do j=1,N
