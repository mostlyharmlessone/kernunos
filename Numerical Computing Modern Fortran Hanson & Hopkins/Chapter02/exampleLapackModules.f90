    PROGRAM exampleLapackModule
! Import the precision parameter for declaring real data
      USE set_precision, ONLY : wp
! Import the names of the Lapack routines used to factorize
! and solve the linear system.
      USE LapackModule, ONLY : dgetrf, dgetrs
! Import the names of the Blas routines used by the main program.
! Other routines are used by the Lapack routines but these
! do not need to be mentioned here.
      USE BlasModule, ONLY : l2Norm=>dnrm2, dgemv
...
! if an illegal input value is detected.
      WRITE (*,'(''Using library of routines stored in a module'')')
      DO WHILE (.TRUE.)
        WRITE (*,'(A)') &
...
! Lapack and the vector used to generate the right hand side.
          ...
          relerr = l2Norm(n,b,1)/l2Norm(n,y,1)
...
! Write out relative errors and timings
    ...
    END PROGRAM exampleLapackModule
