...
!        Default values: epsabs = 0.0_wp, epsrel = sqrt(machine precision)

! Output: INTEGER :: nfunction_evaluations
!        provides the total number of function evaluations performed.

! Output: INTEGER :: nvector_evaluations
!        provides the total number of subintervals used in the evaluation.

! Output: REAL(wp) :: abserr
...
! Input/Output: INTEGER ::ncomponents
...
! comments immediately preceding this type definition.
  ...
  ! Absolute and relative error tolerances required
        ...
        REAL (wp) :: epsrel = sqrt(epsilon(1.0E0_wp))
  ! Total number of function evaluations and subintervals used
        INTEGER :: nfunction_evaluations
        INTEGER :: nvector_evaluations
  ...
  ! Pointer to user-supplied function for norm calculations
      ...
      ABSTRACT INTERFACE
        ...
        RECURSIVE FUNCTION fv(xval,extend) RESULT (yval)
          ...
          REAL (wp), INTENT (IN) :: xval(:)
          CLASS (quadpackbase), INTENT (INOUT) :: extend
          REAL (wp) :: yval(size(xval),extend%ncomponents)
        END FUNCTION fv
...
! If intApprox(:) is rank 1, QAG2003V is called.
      ...
      PUBLIC qag2003, qag2003s, qag2003v, fs, fv, errnorm, wp

! Named constants
      REAL (wp), PARAMETER, PUBLIC :: half = 0.5E0_wp, zero = 0.0E0_wp, &
        one = 1.0E0_wp, two = 2.0E0_wp, onept5 = 1.5_wp, twohun = 200.0_wp, &
        fifty = 50.0E0_wp, cc = 200.0E0_wp, p99 = 0.99_wp, tenm5 = 0.00001_wp, &
        ten4 = 10000.0E0_wp, ten3 = 1000.0E0_wp

! Named constants for linking key values with rules
      INTEGER, PARAMETER :: gk15rule = 1, gk21rule = 2, gk31rule = 3, &
        gk41rule = 4, gk51rule = 5, gk61rule = 6, maxrules = 6

! Array defining number of points associated with each rule
      INTEGER, PARAMETER :: xrule(1:maxrules) = (/ 15, 21, 31, 41, 51, 61 /)

! Indexes for pointers into the one dimension arrays
! to extract each rule
      INTEGER, PARAMETER :: gkindex(1:maxrules+1) = (/ 1, 9, 20, 36, 57, 83, &
        114 /)
      INTEGER, PARAMETER :: gaussindex(1:maxrules+1) = (/ 1, 5, 10, 18, 28, &
        41, 56 /)

! Data for 15-point Gauss-Kronrod rule
      REAL (wp), PARAMETER :: xwg15(1:4) = (/ &
        0.129484966168869693270611432679082E0_wp, &
        0.279705391489276667901467771423780E0_wp, &
        0.381830050505118944950369775488975E0_wp, &
        0.417959183673469387755102040816327E0_wp/)

      REAL (wp), PARAMETER :: xxgk15(1:8) = (/ &
        0.949107912342758524526189684047851E0_wp, &
        0.741531185599394439863864773280788E0_wp, &
        0.405845151377397166906606412076961E0_wp, &
        0.991455371120812639206854697526329E0_wp, &
        0.864864423359769072789712788640926E0_wp, &
        0.586087235467691130294144838258730E0_wp, &
        0.207784955007898467600689403773245E0_wp, &
        0.000000000000000000000000000000000E0_wp/)

      REAL (wp), PARAMETER :: xwgk15(1:8) = (/ &
        0.063092092629978553290700663189204E0_wp, &
        0.140653259715525918745189590510238E0_wp, &
        0.190350578064785409913256402421014E0_wp, &
        0.022935322010529224963732008058970E0_wp, &
        0.104790010322250183839876322541518E0_wp, &
        0.169004726639267902826583426598550E0_wp, &
        0.204432940075298892414161999234649E0_wp, &
        0.209482141084727828012999174891714E0_wp/)

      REAL (wp), TARGET :: xwg(1:gaussindex(maxrules+1)-1) = (/ xwg15, xwg21, &
        xwg31, xwg41, xwg51, xwg61/)
      REAL (wp), TARGET :: xwgk(1:gkindex(maxrules+1)-1) = (/ xwgk15, xwgk21, &
        xwgk31, xwgk41, xwgk51, xwgk61/)
      REAL (wp), TARGET :: xxgk(1:gkindex(maxrules+1)-1) = (/ xxgk15, xxgk21, &
...
! Type and define dummy function values -
  PROCEDURE(fv) :: f
        CLASS (quadpackbase), INTENT (INOUT) :: extype
        REAL (wp), INTENT (IN) :: a, b
        REAL (wp), INTENT (INOUT) :: intapprox(:)
        INTEGER, INTENT (INOUT) :: last
  PROCEDURE(errnorm), POINTER :: norm

        REAL (wp) a12, rl
        REAL (wp) a1, a2, b1, b2, c
        REAL (wp) errbnd, errmax, errsum
        REAL (wp) error1, error2, erro12
        REAL (wp), ALLOCATABLE :: alist(:), blist(:), area(:), area1(:), &
          area2(:), area12(:), resabs(:), elist(:), rlist(:,:), defabs(:), &
          defab1(:), defab2(:)
        INTEGER iroff1, iroff2, keyf, maxerr
        INTEGER nrmax, i
...
! Type and define dummy function values -
  PROCEDURE(FS) :: F
        ...
        REAL (wp) a1, a2, b1, b2, c, defabs, defab1, defab2
        REAL (wp) errbnd, errmax, error1
        REAL (wp) error2, erro12, errsum, resabs
        REAL (wp), ALLOCATABLE :: alist(:), blist(:), elist(:), rlist(:)
        INTEGER iroff1, iroff2, keyf, maxerr
        INTEGER nrmax
        INTEGER, ALLOCATABLE :: iord(:)
...
END ASSOCIATE
      ...
      RECURSIVE SUBROUTINE qkgens(f,extype,a,b,intapprox,resabs,resasc)
        ...
        REAL (wp), INTENT (IN) :: a, b
        REAL (wp), INTENT (INOUT) :: intapprox, resabs, resasc
      PROCEDURE(fs) :: f
        CLASS (quadpackbase), INTENT (INOUT) :: extype
...
!     set the size of the automatic arrays XVALUE, FVALUE.
        ...
        REAL (wp) centr, hlgth, resg, resk, reskh
        INTEGER ng, nk, nt
        REAL (wp), POINTER :: fk1(:), fk2(:), fg1(:), fg2(:)
        REAL (wp), TARGET, DIMENSION (xrule(extype%key)) :: xvalue, fvalue
...
!           THE INTEGRAL, AND ESTIMATE THE ABSOLUTE ERROR.
      ...
      RECURSIVE SUBROUTINE qkgenv(f,extype,a,b,intapprox,resabs,resasc,norm)
        ...
        REAL (wp), INTENT (IN) :: a, b
        REAL (wp), INTENT (INOUT), DIMENSION (:) :: intapprox, resabs, resasc
      PROCEDURE(fv) :: f
        CLASS (quadpackbase), INTENT (INOUT) :: extype
      PROCEDURE(errnorm),POINTER :: norm
...
!                       over (A,B)
        ...
        REAL (wp) centr, hlgth, resg(size(intapprox)), resk(size(intapprox)), &
...
!     set the size of the automatic arrays XVALUE, FVALUE.
        ...
        REAL (wp), TARGET, DIMENSION (xrule(extype%key),size(intapprox)) :: &
...
!    Input/output, integer NRMAX.
        ...
        INTEGER, INTENT (INOUT) :: iord(last), maxerr, nrmax
        REAL (wp), INTENT (INOUT) :: elist(last)
        REAL (wp), INTENT (OUT) :: ermax

        INTEGER i, ibeg, isucc, j, jbnd
        INTEGER jupbn, k, istate
