    PROGRAM documentExample
      ...
      USE normalPdf, ONLY : extype, fmodule
      USE quadpack2003, ONLY : quadpackbase, qag2003, wp, fs
...
! symbols associated with derived types.
      ...
      ASSOCIATE (mu => extend%mu, sigma => extend%sigma)
...
! contains the function fmodule.  
      CALL qag2003(fmodule,extend,mu-one,mu+one,approx1)

! A second packaging method is to use 
! an external function:
      CALL qag2003(fexternal,extend,mu-one,mu+one,approx2)

! A third alternate packaging is to contain the
! evaluation function fcontained in this
! program unit.  
      CALL qag2003(fcontained,extend,mu-one,mu+one,approx3)
      IF (extend%ier==0 .AND. (approx1==approx2) .AND.  &
          (approx1==approx3)) THEN
        WRITE (*,'(A,F5.2,A,F5.2/A,F7.3)') &
          ' Normal cumulative for mu = ', mu,  ' sigma = ', &
          sigma, ' within limits [mu-1,m+1] is = ', approx1
        WRITE(*,*) 'Packaging methods: fmodule, fexternal and fcontained all agree.'
      ELSE
        WRITE (*,'(A)') 'Error -- approximations differ'
        WRITE (*,'(A, E16.8)') 'Using fmodule:    ', approx1
        WRITE (*,'(A, E16.8)') 'Using fexternal:  ', approx2
        WRITE (*,'(A, E16.8)') 'Using fcontained: ', approx3
      END IF
      END ASSOCIATE
...
! s=1/sqrt(2*pi).     
        REAL (wp), PARAMETER :: s = one/sqrt(8.0_wp*atan(one))
